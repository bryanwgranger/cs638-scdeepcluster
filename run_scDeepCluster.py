...
if __name__ == "__main__":
    ...
    parser.add_argument('--conv_sizes', default='256,64',
                        help='sizes for autoencoder, separated by commas')
    ...
    #data_mat = sc.read_10x_h5(args.data_file)
    data_mat = sc.read_10x_mtx(args.data_file)
    # x = np.array(data_mat.X)
    ...
    #adata = sc.AnnData(x)
    #adata = sc.read_10x_h5(args.data_file)
    ...
    print(args)
    conv_sizes = [int(c) for c in args.conv_sizes.split(",")]
...
#    print("median of gene sd: %.5f" % x_sd_median)
    ...
    model = scDeepCluster(input_dim=adata.n_vars, z_dim=32, 
                encodeLayer=conv_sizes, decodeLayer=deconv_sizes, sigma=args.sigma, gamma=args.gamma, device=args.device)
    
    print(str(model))

    t0 = time()
    if args.ae_weights is None:
        model.pretrain_autoencoder(X=adata.X, X_raw=adata.raw.X.todense(), size_factor=adata.obs.size_factors,
    ...
    if args.n_clusters > 0:
        y_pred, _, _, _, _ = model.fit(X=adata.X, X_raw=adata.raw.X.todense(), size_factor=adata.obs.size_factors, n_clusters=args.n_clusters, init_centroid=None,
        ...
        y_pred, _, _, _, _ = model.fit(X=adata.X, X_raw=adata.raw.X.todense(), size_factor=adata.obs.size_factors, n_clusters=n_clusters, init_centroid=cluster_centers,
    ...
    np.savetxt(os.path.join(args.save_dir, args.final_latent_file), final_latent, delimiter=",")
    np.savetxt(os.path.join(args.save_dir, args.predict_label_file), y_pred, delimiter=",", fmt="%i")
